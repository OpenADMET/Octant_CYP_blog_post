...
react_log2["svg"] = react_log2["cxsmiles"].apply(smiles_to_svg)
react_pct["svg"] = react_pct["cxsmiles"].apply(smiles_to_svg)
...
for batch in set(react_log2["ocnt_batch"]) | set(react_pct["ocnt_batch"]):
    _swarm_cache[batch] = make_swarm_svg(batch, peak_areas)

react_log2["swarm_svg"] = react_log2["ocnt_batch"].map(_swarm_cache)
react_pct["swarm_svg"] = react_pct["ocnt_batch"].map(_swarm_cache)
...
<div style="width:340px;">
    <div>@svg{safe}</div>
    <div><b>@batch</b></div>
    <div style="margin-top:4px;">@swarm_svg{safe}</div>
</div>
"""
