...
ic50 = pd.read_csv(repo_root / "data/OPA_C3A4TDIA_005_LMO.csv")
ic50_avg = ic50.groupby("OCNTID").agg(
...
panel_b = ic50_avg.merge(
    ...
    left_on="OCNTID", right_on="ocnt_batch",
    how="inner",
).dropna()
...
def smiles_to_svg(smiles, size=(150, 150)):
...
panel_b["svg"] = panel_b["smiles"].apply(smiles_to_svg)

# --- DRC plot thumbnails as base64 (parallelized) ---

def png_to_base64_thumb(batch_id, width=300):
    path = repo_root / f"data/drc-plots/{batch_id}_c7111_no-ctrls.png"
    if not path.exists():
        return ""
    img = Image.open(path)
...
with ThreadPoolExecutor() as pool:
    panel_b["drc_img"] = list(pool.map(png_to_base64_thumb, panel_b["OCNTID"]))
```

### Panel B: CYP3A4 inhibition vs reactivity

```{python}
#| label: figure-1b-plot
