...
panel_b["mol_img"] = panel_b["standardized_smiles"].apply(smiles_to_svg)

# --- DRC plot thumbnails as base64 (parallelized) ---

def make_drc_thumb(batch_id, width=300):
    ...
    return _save_tooltip(buf.getvalue(), f"drc_{batch_id}.webp")

with ThreadPoolExecutor() as pool:
    panel_b["drc_img"] = list(pool.map(make_drc_thumb, panel_b["ocnt_batch"]))

# --- swarm images ---

panel_b["swarm_img"] = panel_b["ocnt_batch"].map(
    lambda b: save_tooltip_svg(make_swarm_svg(b, peak_areas_b, enzymes=["CYP3A4"]), name=f"swarm_3a4_{b}")
)
```
