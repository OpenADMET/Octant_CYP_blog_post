...
# --- data loading ---
reactivity_raw <- read_csv(here("data", "reactivity.csv"))
inhibition_df <- read_csv(here("data", "inhibition.csv"))
...
# --- Panel A: CYP2J2 log10fc histogram (fewer bins, with counts) ---
hist_a_data <- react_summary |> filter(enzyme == "CYP2J2")
...
panel_a <- hist_a_binned |>
  ...
  labs(
    x = expression(log[10] ~ "(peak area + 1), treatment - control"),
    y = NULL,
    title = "CYP2J2"
  ) +
  theme(
...
# --- Panel B: CYP3A4 log10fc histogram (fewer bins, with counts) ---
hist_b_data <- react_summary |> filter(enzyme == "CYP3A4")
hist_b_binned <- hist_b_data |>
  mutate(
    bin = cut(
      log10fc,
      breaks = seq(
...
hex_theme <- theme(
  ...
  legend.title = element_text(margin = margin(l = -2)),
...
hex_2j2 <- react_summary |>
  ...
  scale_x_continuous(
    labels = label_log10_pseudocount,
    breaks = 0:6,
    limits = c(3, NA)
  ) +
  scale_y_continuous(
  ...
  labs(
    x = "Mean peak area (control)",
    y = "Mean peak area (treatment)",
    fill = "# cpds"
  ) +
  hex_theme +
...
# --- IC50 bin categories (shared by panels C and F) ---
ic50_levels <- c(
  "inactive",
  "10\u2013100 \u00b5M",
  "1\u201310 \u00b5M",
...
inhib_binned <- inhibition_df |>
  filter(!is.na(CYP3A4_pIC50)) |>
  mutate(
    pIC50_plot = if_else(CYP3A4_pIC50 < 4, 2.875, CYP3A4_pIC50),
    ic50_bin = cut(
      CYP3A4_pIC50,
...
# shared x-axis for panels C and F
...
# --- Panel C: pIC50 distribution histogram ---
panel_c <- inhib_binned |>
...
inactive_data <- inhib_react |> filter(CYP3A4_pIC50 < 4)
...
# --- assemble figure ---
fig1 <- (panel_b | panel_a | panel_c) /
  (hex_3a4 | hex_2j2 | panel_d) +
