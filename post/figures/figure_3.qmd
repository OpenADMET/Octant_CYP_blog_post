...
ic50 = pd.read_csv(repo_root / "data/inhibition.csv")
...
peak_areas_b = pd.read_csv(repo_root / "data/reactivity.csv")
...
def png_to_base64_thumb(batch_id, width=300):
    path = repo_root / f"data/drc-plots/{batch_id}_c7111.png"
    if not path.exists():
        return ""
    img = Image.open(path)
...
# --- swarm SVGs (reuses make_swarm_svg & peak_areas from figure_1) ---

panel_b["swarm_svg"] = panel_b["ocnt_batch"].map(
    lambda b: make_swarm_svg(b, peak_areas, enzymes=["CYP3A4"])
)
```
...
def make_panel_b(panel_b):
    ...
    <div style="width:580px;">
        <div style="display:flex; align-items:start; gap:6px;">
            <div>@swarm_svg{safe}</div>
            <div>@svg{safe}</div>
            <div><img src="@drc_img{safe}" width="200"></div>
        </div>
        <div><b>@batch</b> &mdash; pIC50: @pic50{0.2}</div>
    </div>
    """
