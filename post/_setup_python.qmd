...
# Write tooltip images as external assets instead of inlining as base64
TOOLTIP_ASSETS = repo_root / "_site" / "assets" / "tooltips"
TOOLTIP_ASSETS.mkdir(parents=True, exist_ok=True)
TOOLTIP_REL = "assets/tooltips"


def _save_tooltip(data: bytes, filename: str) -> str:
    (TOOLTIP_ASSETS / filename).write_bytes(data)
    return f"{TOOLTIP_REL}/{filename}"
...
def smiles_to_svg(smiles, size=(300, 300)):
    ...
    slug = hashlib.md5(smiles.encode()).hexdigest()[:12]
    return _save_tooltip(svg_str.encode("utf-8"), f"mol_{slug}.svg")


def save_tooltip_svg(svg_str, name):
    if not svg_str:
        return ""
    return _save_tooltip(svg_str.encode("utf-8"), f"{name}.svg")
...
def make_swarm_svg(batch, peak_areas_df, enzymes=None, width=None, height=130):
    ...
    for ei, enz in enumerate(enzymes):
        ...
        parts.append(
            ...
            f' font-size="8" font-family="Arial, sans-serif" fill="{cond_colors["control"]}">control</text>'
        )
        parts.append(
            f'<text x="{trt_x}" y="{height - 5}" text-anchor="middle"'
            f' font-size="8" font-family="Arial, sans-serif" fill="{cond_colors["treatment"]}">treatment</text>'
        )
